...
The selective anchoring principle governs all data movement through the architecture. The end-to-end workflow proceeds as follows (Figure~\ref{fig:smartcontract}):
...
Figure~\ref{fig:web3} illustrates the layered privacy control architecture.

\begin{figure}[!ht]
\centering
\fbox{\parbox{0.8\columnwidth}{\centering\vspace{2em}[Layered Privacy Architecture]\\Figure placeholder\vspace{2em}}}
\caption{Layered privacy architecture: encrypted storage, dynamic access control, and zero-knowledge verification.}\label{fig:web3}
\end{figure}
...
The data analysis component implements three epidemiological dimensions: temporal (day, week, month), spatial (kebele, woreda, town), and person-based (age, sex, occupation) analysis for outbreak detection and disease burden assessment.
...
\textbf{Morbidity measures.} Incidence proportion (new cases relative to the at-risk population) and prevalence (existing cases at a point or period) are computed at Tier~3 coordination nodes.\cite{cdc2012principles} Results are anchored on-chain as verifiable indicator events, supporting outbreak detection decisions grounded in tamper-evident, provenance-tracked epidemiological evidence.

\subsection{Consensus Design and Validator Governance}\label{subsec:consensus}

The architecture requires explicit consensus-selection rationale rather than default adoption of Proof of Authority (PoA). The consensus design addresses three dimensions:
